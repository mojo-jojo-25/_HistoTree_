...
def preparefeatureLabel(batch_graph, batch_label, batch_adjs, name, feature_dim):
...
class Trainer(object):
    ...
    def train(self, sample, model, dim):
        node_feat, labels, adjs, masks, names = preparefeatureLabel(sample['image'], sample['label'], sample['adj_s'], sample['id'], feature_dim=dim)
        cls_token, pred, label, loss, patches = model.forward(node_feat, labels, adjs, masks, names, training=True)

        return cls_token, pred, label, loss, patches
...
class Evaluator(object):
    ...
    def eval_test(self, sample, model, feature_dim, graphcam_flag=False):
        node_feat, labels, adjs, masks, names = preparefeatureLabel(sample['image'], sample['label'], sample['adj_s'],sample['id'], feature_dim)


        if not graphcam_flag:
            with torch.no_grad():
                #pred,labels,loss = model.forward(node_feat, labels, adjs, masks)
                cls_token, pred, label, loss, patches = model.forward(node_feat, labels, adjs, masks,
                                                                                   names,
                                                                                   training=False)
        else:
